  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload4(from11(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload4(from11(envelope)),
...
async function resolveNode() {
  ...
  const { wasmBase64 } = await import("./mine.wasm-MA374HSN.js");
...
function serialize5(authentication, options = {}) {
  ...
  return concat2(serialize(from6(signature)), getFields(authentication));
}
